...
from conductor.coordination.coverage import audit_preferences, canonical_sha256, write_immutable_audit
...
def _tags(state: ExecutionState) -> list[str]:
...
async def generate_preferences(config: dict[str, Any], checkpoint: str | None = None) -> dict[str, Any]:
    ...
    for task in tasks:
        ...
        for step in actor_trajectory.steps:
            state = ExecutionState(**step.state)
            rejected = RoutingDecision(**step.decision)
            ...
            record = {
                ...
                "structure_id": task.metadata["structure_id"], "state_kinds": _tags(state),
            ...
            pair_id = canonical_sha256({"state": record["state"], "chosen": catalog.key(chosen_action),
                                        "rejected": catalog.key(rejected_action),
